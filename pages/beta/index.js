...
const MENU_BY_ROLE = {
  member: [
    { key: 'missions', title: '선교회 운영', desc: '선교회 회원 기준 메뉴와 참여 화면으로 이동합니다.', tone: '#8f693f' },
    { key: 'profile', title: '내 소속', desc: '나의 교회 소속과 역할을 확인합니다.', tone: '#7a6e9e' },
  ],
  leader: [
    { key: 'missions', title: '선교회 운영', desc: '회원, 회비, 일정, 투표를 포함한 선교회 운영 화면으로 이동합니다.', tone: '#8f693f' },
    { key: 'admin', title: '관리 도구', desc: '리더 비밀번호 확인 후 관리 영역으로 들어갑니다.', tone: '#a34d4d' },
  ],
  admin: [
    { key: 'missions', title: '선교회 운영', desc: '권한, 회비, 재정, 문서까지 포함한 운영 기능으로 이동합니다.', tone: '#8f693f' },
      ...
      <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at top right, #eadcc8 0, #f6f1e8 38%, #f5f0e6 100%)' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '28px 18px 64px' }}>
          <div style={{ marginBottom: 18 }}>
            <p style={{ fontSize: 11, letterSpacing: '0.22em', color: '#8b6e4e', fontWeight: 700, margin: '0 0 8px' }}>KWANGHEUNG CHURCH</p>
            <h1 style={{ fontSize: 30, lineHeight: 1.2, margin: '0 0 8px', fontFamily: "'Gowun Batang', serif" }}>광흥교회 통합 앱</h1>
            <p style={{ margin: 0, color: '#6e5b48', lineHeight: 1.7 }}>회원 기반 접속과 권한별 메뉴 구성을 운영 기준으로 확인하는 페이지입니다.</p>
          </div>
            ...
            <div style={{ display: 'grid', gap: 18, gridTemplateColumns: '1.1fr 0.9fr' }}>
              ...
              <div style={{ background: '#fdf7ef', border: '1px solid #eadbc5', borderRadius: 18, padding: 22 }}>
                ...
                <div style={{ display: 'grid', gap: 10, color: '#6e5b48', lineHeight: 1.8 }}>
                  <p style={{ margin: 0 }}>로그인은 `app_members` 테이블 기준으로 동작합니다.</p>
                  ...
                  <p style={{ margin: 0 }}>관리자 비밀번호는 현재 `admin_password_hash` 값을 기준으로 확인합니다.</p>
                </div>
              </div>
            </div>
          ) : (
                ...
                <div style={{ display: 'grid', gap: 18 }}>
                  ...
                  <div style={{ background: '#fff', border: '1px solid #e5d5bd', borderRadius: 18, padding: 20 }}>
                    ...
                    <p style={{ margin: 0, color: '#6e5b48', lineHeight: 1.8 }}>
                      현재 화면은 상단 회원 카드, 좌측 권한별 메뉴, 우측 소속 정보 패널 구조입니다. 메뉴 수와 깊이에 따라 하단 탭 또는 카드형 허브 중 하나로 더 좁히는 것이 좋습니다.
                    </p>
